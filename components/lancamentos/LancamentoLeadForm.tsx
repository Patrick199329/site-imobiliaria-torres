...
interface LancamentoLeadFormProps {
  lancamentoNome: string
  lancamentoSlug: string
}

type Estado = 'idle' | 'loading' | 'success' | 'error'

export function LancamentoLeadForm({ lancamentoNome, lancamentoSlug }: LancamentoLeadFormProps) {
  const [estado, setEstado] = useState<Estado>('idle')
  const [form, setForm] = useState({ nome: '', email: '', telefone: '', mensagem: '' })

  const waUrl = `https://wa.me/5531999999999?text=${encodeURIComponent(`Olá, tenho interesse no empreendimento ${lancamentoNome}.`)}`
    ...
    try {
      const res = await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          mensagem:
            `[Lançamento: ${lancamentoNome} — /lancamentos/${lancamentoSlug}]\n${form.mensagem}`.trim(),
        }),
      })
  ...
  if (estado === 'success') {
    return (
      <div className="flex flex-col items-center gap-3 rounded-md border border-brand-gold/20 bg-brand-navy-deep px-6 py-10 text-center">
        ...
        <p className="font-serif text-lg text-brand-cream">Interesse registrado!</p>
        <p className="font-sans text-sm font-light text-brand-cream/60">
          Nossa equipe entrará em contato em breve com todas as informações sobre{' '}
          <strong className="text-brand-cream/80">{lancamentoNome}</strong>.
        </p>
        <a
          href={waUrl}
          target="_blank"
  ...
  return (
    <div className="rounded-md border border-brand-cream/10 bg-brand-navy-deep p-6">
      <h3 className="mb-1 font-serif text-lg font-semibold text-brand-cream">Quero saber mais</h3>
      <p className="mb-5 font-sans text-sm font-light text-brand-cream/50">
        Receba todas as informações sobre {lancamentoNome}.
      </p>
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-3">
        ...
        />
        <textarea
          placeholder="Qual tipologia te interessa? (opcional)"
          rows={3}
          value={form.mensagem}
        ...
        >
          {estado === 'loading' ? 'Enviando…' : 'Registrar interesse'}
        </button>
      </form>
